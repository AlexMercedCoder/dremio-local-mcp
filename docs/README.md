# Dremio Local MCP Documentation

Welcome to the documentation for the Dremio Local MCP server.

## Index

### CLI
- [CLI Command Reference](cli/README.md)

### Tools
- [Catalog Tools](tools/catalog.md) (Datasets, Schemas, Context)
- [Semantic Tools](tools/semantic.md) (Views, Wikis, Tags, Planning)
- [Query Tools](tools/query.md) (SQL Execution)
- [Jobs & Reflection Tools](tools/jobs_and_reflection.md) (Analysis, Performance)
- [Docs Tools](tools/docs.md) (Local Search)

### Prompts
- [Prompt Reference](prompts/README.md) (Analysis, Optimization, Ops)

### Configuration
- [Client Configuration](clients.md) (Claude Desktop, etc.)
